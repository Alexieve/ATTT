...
CREATE OR REPLACE PROCEDURE USP_KHMO_GETALL_TRUONGKHOA(P_RES OUT SYS_REFCURSOR)
AS
BEGIN
    OPEN P_RES FOR
        SELECT KHMO.HK, KHMO.NAM, HP.MADV, HP.TENHP, HP.MAHP, KHMO.MACT
        FROM KHMO JOIN HOCPHAN HP
        ON KHMO.MAHP = HP.MAHP
        ORDER BY NAM DESC, HK DESC, HP.MADV, HP.MAHP;
END;
/
GRANT EXECUTE ON USP_KHMO_GETALL_TRUONGKHOA TO PUBLIC;